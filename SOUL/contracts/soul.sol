...
contract TITANS is ERC20, Ownable, ERC20Burnable, IERC721Receiver {
    ...
    constructor() ERC20("TITANS", "TITANS")   {

        nonfungiblePositionManager = INonfungiblePositionManager(NONFUNGIBLE_POSITION_MANAGER_ADDRESS);
...
//    }
    ...
    function swapExactInputSingle(uint256 amountIn, uint24 fee,uint256 amountOutMinimum, uint160 sqrtPriceLimitX96) external returns (uint256 amountOut) {
        // msg.sender must approve this contract

        ISwapRouter swapRouter = ISwapRouter(ROUTER_ADDRESS);
        address DAI = address(this);
...
contract StakingContract is Pausable, Ownable {
    ...
    constructor(address _SOUL, address _TITANS)   {
        SOUL = IERC20(_SOUL);
